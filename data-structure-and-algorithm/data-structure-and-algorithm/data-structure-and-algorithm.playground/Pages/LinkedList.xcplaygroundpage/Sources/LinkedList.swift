...
//
//  单向链表(Single linked list)：单向链表的链接方向是单向的，每个节点都有指针成员变量指向列表中的下一个节点；
//  双向链表(Double linked list)：双向链表的每个节点中都有两个指针，分别指向直接后继和直接前驱。 所以，从双向链表中的任意一个节点开始，都可以很方便地访问它的前驱节点和后继节点，它的链接方向是双向的。
//  循环链表(Circular linked list)：循环链表是另一种形式的链式存贮结构。它的特点是表中最后一个节点的指针域指向头节点，整个链表形成一个环。
...
/*
 关于Dummy节点
 上面代码引入了Dummy节点，它的作用就是作为一个虚拟的头前节点。
 我们引入它的原因是我们不知道要返回的新链表的头节点是哪一个，它有可能是原链表的第一个节点，可能在原链表的中间，也可能在最后，甚至可能不存在（nil）。
 而Dummy节点的引入可以巧妙的涵盖所有以上情况，我们可以用dummy.next方便得返回最终需要的头节点。
*/
